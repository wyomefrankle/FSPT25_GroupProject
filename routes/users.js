...
router.get('/', async (req, res) => {
  ...
  const prompt = `Give me recommendations for skincare products for ${skintype} skin that targets ${skinconcern} that is available in the ${country} and less than ${budget} USD and accessible in most stores. I want 3 product recommendations for a cleanser, 3 recommendations for a toner, 3 recommendations for a serum, 3 recommendations for a face cream. Note: Please make the output be in an easy to use object format that I can post to the backend of a full stack js app that has an SQL table like this: CREATE TABLE SkincareProducts (id INT PRIMARY KEY AUTO_INCREMENT, product_type VARCHAR(50) NOT NULL, brand VARCHAR(255) NOT NULL, name VARCHAR(255) NOT NULL, price DECIMAL(5,2) NOT NULL, description TEXT, imageURL TEXT);`

  const result = await model.generateContent(prompt);
  const response = await result.response;
  const text = response.text();
  res.json( {text} );
  console.log({text}) //if i want to see the text returned by Gemini in terminal
} catch (error) {
